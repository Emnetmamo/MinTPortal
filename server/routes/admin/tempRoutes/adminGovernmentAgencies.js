...
import Agencies from '../../../models/tempModel/agencies.js'
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const newAgenciesId = 'image-' + Date.now();
    const newsAgenciesPath = `public/agencies_images/${newAgenciesId}`;
    const galleryPath = path.join(newsAgenciesPath, 'gallery');
    const thumbsPath = path.join(galleryPath, 'thumbs');

    // Create directories if they don't exist
    [newsAgenciesPath, galleryPath, thumbsPath].forEach((dirPath) => {
      fs.mkdirSync(dirPath, { recursive: true });
    });

    cb(null, newsAgenciesPath); // Set the destination path
  },
...
// POST add-news
router.post('/government-agencies',Verify, (req, res) => {
  upload(req, res, async (err) => {
    if (err) {
      ...
      try {
          const newAgencies =  new Agencies({
            title,
            link,
            description,
            email,
       ...
       const savedAgencies = await newAgencies.save();
        res.json(savedAgencies);
